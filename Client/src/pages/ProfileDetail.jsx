...
function ProfileDetail() {
  ...
  const [parentsStatus, setParentsStatus] = useState({ mother: 'alive', father: 'alive' });
  const [familyHealthIssues, setFamilyHealthIssues] = useState(false);
  ...
  useEffect(() => {
    const CheckUserSubInfo1 = async () => {
      try {
        const token = localStorage.getItem('Authorization');
        const response = await api.get('/api/users/check/usersub/info1', {
          headers: {
            Authorization: token,
          }
        });
  ...
  const nextStep = () => {
    if (validateStep()) {
      ...
      setTimeout(() => {
        setLoading(false);
        setStep(step + 1);
        setError(''); // Reset error on valid step
      }, 1000);
    }
  };
  ...
  const validateStep = () => {
    switch (step) {
      case 1:
        if (!birthDate) {
          handleAlert('warning', 'Please select your birth date.');
          return false;
        }
        const calculatedAge = calculateAge(birthDate);
        setAge(calculatedAge);
        if (calculatedAge < 15) {
          handleAlert('warning', 'You are too young to make financial decisions.');
          return false;
        }
        return true;
      case 2:
        if (!maritalStatus) {
          handleAlert('warning', 'Please select your marital status.');
          return false;
        }
        return true;
      case 3:
      ...
      case 4:
        if(childrenCount > 0) {
          for (let i = 0; i < childrenCount; i++) {
            ...
            if (!child.age) {
              handleAlert('warning', 'Please fill out the age for all children.');
              return false;
            }
            if (child.age >= 7 && !child.education) {
              handleAlert('warning', 'Please select education level for children over 7 years old.');
              return false;
            }
          }
        }
      ...
      case 5:
        if (!city) {
          handleAlert('warning', 'Please enter the city you live in.');
          return false;
        }
        return true;
      default:
  ...
  const handleChildrenDetailsChange = (index, field, value) => {
    ...
    updatedDetails[index] = { ...updatedDetails[index], [field]: value };  // Prevent undefined errors
    setChildrenDetails(updatedDetails);
  };

  const handleAddChild = () => {
    if (childrenCount > 0) {
      setChildrenDetails([...childrenDetails, { age: '', education: '' }]);
    }
  };
  ...
  const handleChildrenCountChange = (e) => {
    ...
    if (count === 0) {
      setChildrenDetails([]); // Clear children details when count is 0
    } else {
      // Ensure there are enough child details
      const updatedDetails = [...childrenDetails];
      while (updatedDetails.length < count) {
        updatedDetails.push({ age: '', education: '' });
      }
      setChildrenDetails(updatedDetails);
    }
  };
  ...
  return (
    <div className="profile-detail-container">
      ...
      {loading ? (
        ...
        <>
          {step === 1 && (
            <div className="step-section fadeIn">
          ...
          {step === 2 && (
            <div className="step-section fadeIn">
              ...
              <progress class="progress-bar" value="35" max="100"></progress>
              <select
              ...
              >
                ...
                <option value="partner-dead">Partner Deceased</option>
              </select>
              <div className="error-message">{error}</div>
              <button className="next-button" onClick={nextStep}>
                Next
          ...
          {step === 3 && maritalStatus === 'single' && (
            <div className="step-section fadeIn">
              <h2>Enter details about your parents</h2>
              <progress class="progress-bar" value="70" max="100"></progress>
              <div className="parent-details">
                <h3>Mother's Status</h3>
                <select
                  className="input-select"
                  onChange={(e) => setParentsStatus({ ...parentsStatus, mother: e.target.value })}
                  value={parentsStatus.mother}
                >
                ...
                <select
                  className="input-select"
                  onChange={(e) => setParentsStatus({ ...parentsStatus, father: e.target.value })}
                  value={parentsStatus.father}
                >
                  <option value="alive">Alive</option>
                  <option value="deceased">Deceased</option>
                </select>
                <div className="error-message">{error}</div>
                <button className="next-button" onClick={nextStepplus}>
                  Next
                </button>
              </div>
            </div>
          )}
          {step === 4 && maritalStatus === 'married' && (
            <div className="step-section fadeIn">
              <h2>Enter details about your parents</h2>
              <progress class="progress-bar" value="70" max="100"></progress>
          ...
          )}{step === 4 && maritalStatus === 'partner-dead' && (
            <div className="step-section fadeIn">
              <h2>Enter details about your parents</h2>
              <progress class="progress-bar" value="70" max="100"></progress>
              <div className="parent-details">
          ...
          {step === 4 && childrenCount > 0 && (
            <div className="step-section fadeIn">
              <h2>Enter details about your children</h2>
              ...
              {Array.from({ length: childrenCount }).map((_, index) => (
                <div key={index} className="child-details">
                  ...
                  <input
                    ...
                    type="number"
                    placeholder="Age"
                    value={childrenDetails[index]?.age || ''}
                    onChange={(e) => handleChildrenDetailsChange(index, 'age', e.target.value)}
                  />
                  {childrenDetails[index]?.age >= 7 && (
                    <select
                      className="input-select"
                      value={childrenDetails[index]?.education || ''}
                      onChange={(e) => handleChildrenDetailsChange(index, 'education', e.target.value)}
                    >
                      <option value="">Select Education Level</option>
                      <option value="1-5th">1-5th</option>
                      <option value="5-10th">5-10th</option>
...
/>
    ...
    <button id='submit88' className='next-button' onClick={handleSubmit()}>SUBMIT</button>
  </div>
)}
        </>
      )}
